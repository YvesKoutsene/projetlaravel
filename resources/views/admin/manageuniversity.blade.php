        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                            ...
                            @foreach ($universities as $university)
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900 dark:text-gray-300 text-center couleur">{{ $university->name }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900 dark:text-gray-300 text-center couleur">{{ $university->location}}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900 dark:text-gray-300 text-center couleur">{{ $university->website}}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900 dark:text-gray-300 text-center couleur">{{ $university->description}}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center justify-center">
                                            <img src="{{ asset($university->photo) }}" class="w-10 h-12 rounded-full object-contain" alt="University Logo">
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 text-center">
                                        <div class="flex justify-center items-center space-x-4">
                                            <form id="deleteForm{{ $university->id }}" action="{{ route('delete.university', $university->id) }}" method="POST" class="p-4">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button" onclick="confirmDelete({{ $university->id }})" class="bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-4 rounded couleurb1 transition duration-300 ease-in-out" title="Delete University">
                                            ...
                                            <form action="{{ route('universities.edit', ['id' => $university->id]) }}" method="GET" class="p-4">
                                                @csrf
                    ...
                    @endif
                    <div>
                        <form action="{{ route('universities.create') }}" method="GET">
                            @csrf
                            @method('GET')
